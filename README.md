...
- **学术文献检索**：集成 arXiv 与 Crossref，结构化提取论文元数据并自动补全 DOI。
- **引用可信控制**：按 DOI、arXiv ID 和标题去重，进行来源评分并生成学术引用白名单。
...
7. `normalize_academic_sources` 解析论文记录，补全 Crossref 元数据并执行去重、评分和引用验证。
...
│   │   ├── academic_tools.py    # arXiv/Crossref、论文去重评分和引用验证
...
学术研究第一版还提供 `Academic Search Enabled`、`Crossref Enrichment Enabled`、`Citation Verification Enabled`、`Max Papers Per Query`、`Max Academic Papers` 和可选年份范围配置。
...
- [x] 接入 arXiv 与 Crossref 学术检索
- [x] 增加论文去重、来源质量评分与引用白名单检查
- [ ] 接入 Semantic Scholar 学术检索
